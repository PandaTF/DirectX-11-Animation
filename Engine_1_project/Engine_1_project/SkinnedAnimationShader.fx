...
PS_INPUT VS(VS_INPUT input)
{
    ...
    {
		if(input.jointIndices[i] != -1)
			skinned_pos += mul(output.Pos, translateMatrix[input.jointIndices[i]]) * input.weights[i];
    }
    output.Pos = skinned_pos;
